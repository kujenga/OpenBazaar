...
if [[ $MODE == production ]]; then
    ...
    $PYTHON node/tornadoloop.py $STOREFILE $PRIMARY_MARKET_IP -n 47771 -l $LOGDIR/demo_node1.log -u 1 &
    
    # Demo Peer Market
    sleep 2
    STOREFILE2=ppl/s_tec
    $PYTHON node/tornadoloop.py $STOREFILE2 127.0.0.2 -s tcp://$PRIMARY_MARKET_IP:$MY_MARKET_PORT -n 47772 -l $LOGDIR/demo_node2.log -u 2 &
    
    sleep 2
    STOREFILE3=ppl/genjix
    $PYTHON node/tornadoloop.py $STOREFILE3 127.0.0.3 -s tcp://$PRIMARY_MARKET_IP:$MY_MARKET_PORT -n 47773 -l $LOGDIR/demo_node3.log -u 3 &
    
    sleep 2
    STOREFILE4=ppl/novaprospekt
    $PYTHON node/tornadoloop.py $STOREFILE4 127.0.0.4 -s tcp://$PRIMARY_MARKET_IP:$MY_MARKET_PORT -n 47774 -l $LOGDIR/demo_node4.log -u 4 &
